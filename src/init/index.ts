    ...
    */
    ...
    if (!await dirExists(`/home/${user}`)) {
        ...
        apps.forEach(async (app, i) => {
            ...
            await Filer.promises.writeFile(`/apps/system/${name}.tapp/index.json`, JSON.stringify({
                name: app.name,
                config: app.config,
        ...
        await Filer.promises.writeFile(`/apps/user/${user}/files/config.json`, JSON.stringify({
            "quick-center": true,
            "sidebar-width": 180,
            "drives": {
        ...
        await Filer.promises.writeFile(`/system/etc/terbium/file-icons.json`, JSON.stringify({
            "ext-to-name": dat["ext-to-name"],
            "name-to-path": iconArrays
        }))
        await Filer.promises.writeFile(`/apps/user/${user}/files/quick-center.json`, JSON.stringify({
            "paths": {
